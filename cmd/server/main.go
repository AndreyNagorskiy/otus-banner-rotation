...
func main() {
	...
	amqpClient, err := amqp.NewClient(cfg.MakeAMQPConnectionString())
	if err != nil {
		l.Error("Failed to create RabbitMQ client", slog.String("error", err.Error()))
		return
	}
	defer amqpClient.Close()

	bannerRotation := app.New(l, rep, &b, amqpClient)
